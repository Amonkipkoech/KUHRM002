page 85500 "HRM-Approved Requisition Card"
{


    PageType = Document;
    InsertAllowed = false;
    PromotedActionCategories = 'New,Action,Process,Job,CIR,Approval,Post,Report,Print/send,Advertise';
        ...
        area(navigation)
        {
            ...
            group(CIRO)
            {
                action("&CIR Submission")
                {
                    ...
                    begin
                        if Confirm('Send to CIR', false) = false then begin exit end;
                        Rec.TestField("Requisition No.");
                        Rec."Requisition Status" := Rec."Requisition Status"::CIRO;
                        Rec.CIRO := true;
                        Rec.Advertised := false;
                        Rec.Modify();
