    ...
    begin
        ...
        begin
            ...
            if HREmp2.Get(strEmpCode) then begin
                if HREmp2."Contract Type" = 'Permanent' then begin
                    strTransDescription := 'Basic Pay';
                    TGroup := 'EARNINGS';  //uncommented and chaged to EARNINGS-HOSEA
                end;

                if HREmp2."Contract Type" = 'Contract' then begin
                    strTransDescription := 'Basic Pay';
                    TGroup := 'BASIC SALARY';
                end;
        ...
        JournalPostAs::Credit, JournalPostingType::"G/L Account", '', CoopParameters::NSSF);
            ...
            IF curPensionStaff > 0 THEN BEGIN
                ...
                ELSE
                    curTransAmount := curPensionStaff;
                strTransDescription := 'Provident Relief';
                TGroup := 'TAX COMPUTATION';
                TGroupOrder := 6;
                TSubGroupOrder := 2;
                fnUpdatePeriodTrans(strEmpCode, 'PNSR', TGroup, TGroupOrder, TSubGroupOrder, strTransDescription,
                ...
                EmpPensionContrib := curBasicPay * (0.06);
                //Employer pension Contribution
                fnUpdatePeriodTrans(strEmpCode, 'NEmprPension', 'NEmprPension', 14, 0, 'Employer Provident', EmpPensionContrib, 0, intMonth, intYear, '', '', SelectedPeriod, Dept, '', JournalPostAs::" ", JournalPostingType::" ", '', CoopParameters::none);
            ...
            PREmpTrans_2.SetFilter("Transaction Code", '%1', 'DS-056');
            if PREmpTrans_2.Find('-') then begin


                fnUpdatePeriodTrans(strEmpCode, 'HE-Levi', 'HE-Levi', 16, 0, 'Housing Levi Contrib Employer', curGrossPay * 0.015 + PREmpTrans_2.Amount, 0, intMonth,
...
*/
                    ...
                    if prTransactionCodes.Find('-') then begin
                        ...
                        if (prTransactionCodes."Special Transactions" = prTransactionCodes."Special Transactions"::"Defined Contribution") then
                            if prTransactionCodes.Pension = true then
                             ...
                             begin
                                curTransAmount := (0.05 * curBasicPay);//-curNSSF;
                                                                       //curTransAmount := 0;
    ...
    var
        ...
        P9OtherInsuranceRelief: Decimal;
        P9TaxRelief: Decimal;
        P9Paye: Decimal;
        P9NSSF: Decimal;
        P9provident: Decimal;
                        ...
                        case prPeriodTransactions."Group Order" of
                            ...
                            6: //Taxation
                                begin
                                    ...
                                    if prPeriodTransactions."Sub Group Order" = 12 then P9InsuranceRelief := prPeriodTransactions.Amount; //Insurance Relief
                                    if prPeriodTransactions."Sub Group Order" = 8 then P9OtherInsuranceRelief := prPeriodTransactions.Amount;//otherinsurance relief
                            ...
                            8://Deductions
                                begin
                                    if prPeriodTransactions."Transaction Code" = 'D-20' then P9provident := prPeriodTransactions.Amount; //provident
    ...
    begin
        ...
        prEmployeeP9Info.Pension := providentContr;
        prEmployeeP9Info."OtherInsurance Relief" := OtherIns;
